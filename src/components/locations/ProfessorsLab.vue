...
<template>
    <div class="location-details bg-pokemon-dark p-8 text-white text-center max-w-full max-h-screen overflow-y-auto">
      ...
      <div class="flex items-center justify-center space-x-4">
        <img v-for="(pokemonDetails, index) in selectedPokemonDetails" :key="index" :src="pokemonDetails?.sprites?.front_default" :alt="pokemonDetails?.name" class="w-44 h-44">
      </div>
  ...
  export default {
    ...
    async mounted() {
      // Fetch details for the first, fourth, and seventh Pokémon from the Pokedex store
